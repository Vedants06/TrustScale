            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_400px]">
                ...
                <div className="space-y-6">
                                ...
                                <div className="flex gap-2">
                                    {SEAT_CLASSES.map((cls) => (
                                        <button
                                            ...
                                            className={`rounded-md border px-4 py-2 text-sm font-medium transition-colors ${selectedClass === cls
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        ...
                        <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4 text-center">
                            <div className="text-2xl font-bold text-emerald-500">{totalSuccessCount}</div>
                            <div className="text-xs text-muted-foreground">Success</div>
                        </div>
                        <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-4 text-center">
                            <div className="text-2xl font-bold text-red-500">{totalFailCount}</div>
                            <div className="text-xs text-muted-foreground">Failed</div>
                        </div>
                        <div className={`rounded-lg border p-4 text-center ${avgResponseTime > 1000
                                ? "border-red-500/30 bg-red-500/10"
                                : avgResponseTime > 500
                                    ? "border-amber-500/30 bg-amber-500/10"
                                    : "border-border bg-card"
                            }`}>
                            <div className={`text-2xl font-bold ${avgResponseTime > 1000
                                    ? "text-red-500"
                                    : avgResponseTime > 500
                                        ? "text-amber-500"
                                        : ""
                ...
                <div className="rounded-lg border border-border bg-card">
                    <div className="border-b border-border p-4">
                        ...
                        <p className={`text-xs font-medium ${successRate >= 95
                            ? "text-emerald-400"
                            : successRate >= 80
                                ? "text-yellow-400"
                                : successRate >= 60
                        ...
                        {bookingHistory.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                {bookingHistory.map((booking) => (
                                    <div
                                        key={booking.id}
                                        className={`rounded-md border p-3 text-sm ${!booking.success
                                            ? "border-red-500/30 bg-red-500/5"
                                            : booking.response_time_ms > 2000
                                                ? "border-red-500/30 bg-red-500/5"
                                                : booking.response_time_ms > 1000
                                                    ? "border-amber-500/30 bg-amber-500/5"
                                                    : booking.response_time_ms > 500
